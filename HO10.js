...
if (stringPassword.length <= 8) // checking the first criteria that the password has more than 8 digits. 
    {
        stringOutput = "This Password does not meet the criteria"; // Output info for the user if the entered password is not too 8 digits long
    }
    else
   {
   for (var a = 1; stringPassword <= arrayBadPasswords.length; a++) //loop to check the array
   {
    if (stringPassword === arrayBadPasswords[a])
    // checking second criteria that the password is not too easy ot is one password entered in the array
    {
      stringOutput = "This Password does not meet the criteria. It is too easy."; // Text Output info for the user
    }
      break;
    }
  };
  
  // alert for the user if the password meets the criteria or not
  if (stringOutput != "") 
  {
  ...
  {
      $("output").value = "This password meets the criteria.";
  }
};

// push the DOM input back into the html
